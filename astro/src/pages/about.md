...
12 月 9 日，实现 HGPT、HGAR 图像导入导出。

12 月 10 日，实现 TEXT 文本导入导出。

12 月 12 日，实现 BIND 文件的处理。

12 月 15 日，生成 GUI 程序。

2026 年 1 月 16 日，`Frykte` 加入项目。

1 月 22 日，使用 Qwen3-VL 进行图片文字识别；进行性能优化，将导入/导出时间压缩到 30 秒左右。

1 月 24 日，添加开始菜单。

1 月 26 日，劫持存档函数，将语言参数修改为简体中文。

1 月 28 日，支持 CMake 构建，移除 Font_Hack 依赖。

1 月 31 日，添加 ULJS00061 补丁生成。

2 月 8 日，添加贡献者启动页面。

3 月 4 日，插件版补丁可行性验证。

3 月 12 日，`pan1927412` 加入项目。

4 月 27 日，汉化测试版 `v1.0.0-beta.1` 发布，`blabubble` 加入项目。

4 月 28 日，汉化届传奇人物 TPU 标星了本仓库！

5 月 27 日，汉化测试版 `v1.1.0-beta.1` 发布。

5 月 29 日，`Katsumi-Mole` 加入项目。

6 月 5 日，将补丁架构更改为启动器与运行时插件，通过修改语序实现记忆文本的汉化。

6 月 13 日，修复机密情报显示问题，为翻译文本添加检查和自动修复工具。

6 月 26 日，添加汉化组滚动职员表，发布 `v2.0.0-beta.1`。

7 月 26 日，发布 `v2.0.1-beta.1`。

7 月 27 日，发布 `v2.0.1-beta.2`，修复伊吹摩耶剧情无法进入的问题。

7 月 28 日，发布 `v2.1.0-beta.1`，实现流式生成，补丁体积从 150MB 缩减到 10MB。

7 月 30 日，发布 `v2.1.1-beta.1`，紧急修复 CEV 换行符问题。
