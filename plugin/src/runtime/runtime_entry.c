...
int module_start(SceSize args, void *argp)
{
    ...
    RuntimeLog_Init();
    RuntimeLog_Printf("runtime module_start args=%u", (unsigned)args);
    if (args < sizeof(start_args) || !argp) {
        RuntimeLog_Printf("runtime module_start: invalid args or argp");
        return 0;
    }
    ...
    if (start_args.boot_mid < 0) {
        RuntimeLog_Printf("runtime module_start: boot_mid < 0");
        return 0;
    }

    info.size = sizeof(info);
    if (sceKernelQueryModuleInfo(start_args.boot_mid, &info) < 0) {
        RuntimeLog_Printf("runtime module_start: sceKernelQueryModuleInfo failed for %d", start_args.boot_mid);
        return 0;
    }
